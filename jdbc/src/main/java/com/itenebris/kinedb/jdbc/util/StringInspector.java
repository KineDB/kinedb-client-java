...
public class StringInspector {
        ...
        if (source == null) {
            ...
            if (this.defaultSearchMode.contains(SearchMode.SKIP_BETWEEN_MARKERS)) {
                if (this.openingMarkers == null || this.closingMarkers == null || this.openingMarkers.length() != this.closingMarkers.length()) {
                    throw new IllegalArgumentException("Illegal argument value {0} for openingMarkers and/or {1} for closingMarkers. These cannot be null and must have the same length. \n" + new String[]{this.openingMarkers, this.closingMarkers});
                }

                if (this.overridingMarkers == null) {
                    throw new IllegalArgumentException("Illegal argument value {0} for overridingMarkers. These cannot be null and must be a sub-set of openingMarkers .\n" + new String[]{this.overridingMarkers, this.openingMarkers});
                }
                ...
                for(int var9 = 0; var9 < var8; ++var9) {
                    ...
                    if (this.openingMarkers.indexOf(c) == -1) {
                        throw new IllegalArgumentException("Illegal argument value {0} for overridingMarkers. These cannot be null and must be a sub-set of openingMarkers {1}.\n" + new String[]{this.overridingMarkers, this.openingMarkers});
                    }
                }
            }
    ...
    public int setStartPosition(int pos) {
        if (pos < 0) {
            throw new IllegalArgumentException("The start position must be zero or a positive number.");
        } else if (pos > this.stopAt) {
            throw new IllegalArgumentException("The start position must must not be higher than the stop position.");
        } else {
    ...
    public int setStopPosition(int pos) {
        if (pos < 0) {
            throw new IllegalArgumentException("The stop position must be zero or a positive number.");
        } else if (pos > this.srcLen) {
            throw new IllegalArgumentException("The stop position must must not be higher than the length of the source string.");
        } else {
    ...
    public int matchesIgnoreCase(String... toMatch) {
        if (toMatch == null) {
            ...
            int toMatchLength = 0;
            String[] var3 = toMatch;
            int localStopAt = toMatch.length;

            for(int var5 = 0; var5 < localStopAt; ++var5) {
                String toMatchPart = var3[var5];
            ...
            if (toMatchLength == 0) {
                ...
                localStopAt = this.srcLen - toMatchLength + 1;
                if (localStopAt > this.stopAt) {
                    localStopAt = this.stopAt;
                }
    ...
    public List<String> split(String delimiter, boolean trim) {
        if (delimiter == null) {
            throw new IllegalArgumentException("The delimiter string must not be null.");
        } else {
            this.restart();
            int startPos = 0;
